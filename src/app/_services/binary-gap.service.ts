 ...
 * and contains a binary gap of length 2. The number 529 has binary representation 1000010001 and contains
 * two binary gaps: one of length 4 and one of length 3. The number 20 has binary representation 10100 and
...
export class BinaryGapService {

  constructor() { }

  run(integer: number): number {
    ...
    for (let index = 0; index < binary.length; index++) {
      const bit = binary.substr(index, 1);
      if (bit === '1') {
        hasOne = true;
        if (zeroes > maxGap) {
          maxGap = zeroes;
